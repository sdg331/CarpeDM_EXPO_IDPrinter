...
2026-09-15 사용자 선택 방향: **화이트 바탕, 선명한 포인트색, 간결한 구성**.
최종 프론트엔드는 화이트 `#ffffff`, 블루 `#2864ed`, 본문 `#191f28`, 보조 글씨 `#5c6675`, 중립 표면 `#f5f6f8`, 경계 `#e5e8eb`를 사용한다. 브랜드·주 행동·현재 상태에 블루를 집중한다. 행사 인쇄물의 최종 브랜드 가이드는 별도다.
...
프론트엔드는 Pretendard Variable v1.3.9 WOFF2를 프로젝트에 포함하고 로컬에서 제공한다. 라이선스와 출처는 `frontend/assets/fonts/`에 보관한다. 배지 출력에 사용하는 OS 폰트는 별도로 Pi에서 검증한다.

2026-09-15 구현 조정 — 10.1인치 화면 가독성:
- 800×1280 원본 기준 본문 28px, 보조 안내 24px, 제목 52px, 홈 제목 60px.
- 주요 버튼 글씨 32px / 높이 104px 이상. 홈은 36px 이름과 24px 설명, 입사 144px / 퇴근 124px 높이로 주 행동을 구분한다.
...
현재 시안 구성:
- 홈은 왼쪽 정렬한 큰 제목, 평면 사원증 예시 한 장, 블루 입사 버튼과 흰색 퇴근 버튼을 사용한다. 예시는 실물 인쇄 레이아웃과 구분해 표시한다.
- 팀 선택은 확정된 6개 팀에 짧은 소개와 단색 아이콘을 표시한다. 상세는 소개·키워드 카드와 주요 업무 카드로 구분한다. 얇은 테두리와 14~16px 모서리를 사용한다.
- 다른 단계의 버튼·진행 표시·촬영 안내·결과 카드는 같은 블루 톤을 사용한다. 오류 안내는 빨간 계열로 구분한다.
- AI 선택도 얇은 테두리와 중립 표면을 사용하고 겹침·회전 장식을 줄인다. 대기 문구는 실제 처리 내용으로 작성한다.
...
- `prefers-reduced-motion`에 따라 CSS 효과와 JS 화면 이동을 생략한다. 사용 중 설정이 바뀌면 진행 중 효과도 취소한다.

최종 구현: 화면 이동 280ms/12px, 홈 복귀 160ms, 터치 눌림 120ms/0.985배·복귀 220ms, 모달 진입 240ms·닫힘 140ms, 완료 체크 360ms. 프로젝트에서 정한 값이며 제공된 Apple·Toss 문서의 공식 토큰으로 주장하지 않는다.

동일 화면의 상태 갱신은 화면 전환을 다시 재생하지 않는다. 빠른 이동은 이전 효과를 취소하며, 모달 닫힘은 입력 차단을 즉시 해제한다. 버튼은 모션 감소 설정에서도 눌림 색으로 피드백한다. 세부 결정과 검증은 `docs/FRONTEND_FINAL.md`에 기록한다.
