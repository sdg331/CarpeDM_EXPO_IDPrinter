...
function home() {
  return `<div class="home-head"><h1>나만의 사원증으로<br>체험을 시작해보세요.</h1><p class="subtitle">팀을 고르고, AI 프로필을 만들어요.</p></div>
    <div class="home-visual"><div class="sample-card" role="img" aria-label="사원증 디자인 예시. 샘플 프로필, 김미래, 디자인팀."><div class="sample-card-header" aria-hidden="true">${logo()}<span>MIRRORTING WORKS</span><span>사원증</span></div><div class="sample-card-body" aria-hidden="true"><div class="sample-photo"><img src="/assets/characters/char_01.png" alt=""></div><div class="sample-card-info"><strong>김미래</strong><span>디자인팀</span><small>사원증 디자인 예시</small></div></div></div><div class="home-steps" aria-label="입사 체험 순서"><span>팀 선택</span>${icon("arrow")}<span>AI 프로필</span>${icon("arrow")}<span>카드 등록</span></div></div>
    <div class="entry-grid"><button class="entry-card primary" data-action="checkin"><span class="entry-icon">${icon("badge")}</span><span class="entry-copy"><strong>입사하기</strong><span>나만의 사원증 만들기</span></span>${icon("arrow", "entry-arrow")}</button><button class="entry-card secondary" data-action="checkout"><span class="entry-icon">${icon("logout")}</span><span class="entry-copy"><strong>퇴근하기</strong><span>오늘의 체험 리포트 받기</span></span>${icon("arrow", "entry-arrow")}</button></div><div class="home-bottom">사원증을 만든 후 스마트미러로 이동하세요.</div>`;
}
