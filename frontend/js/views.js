...
function home() {
  return `<div class="home-head"><div class="eyebrow">A NEW DAY. A NEW POSSIBILITY.</div><h1>오늘, 어떤 하루를<br>시작할까요?</h1><p class="subtitle">또 다른 나를 발견하는 곳, MIRRORTING WORKS.<br>당신의 새로운 출근을 환영합니다.</p></div>
    ...
    <div class="entry-grid"><button class="entry-card primary" data-action="checkin"><span class="entry-top">START YOUR DAY ${icon("badge")}</span><h2>입사하기</h2><p>나만의 AI 프로필과<br>사원증을 만들어보세요.</p>${icon("arrow", "entry-arrow")}</button><button class="entry-card" data-action="checkout"><span class="entry-top">WRAP UP YOUR DAY ${icon("logout")}</span><h2>퇴근하기</h2><p>오늘의 경험을 담은<br>퇴근 리포트를 받아보세요.</p>${icon("arrow", "entry-arrow")}</button></div><div class="home-bottom">${icon("clock")} 입사 등록 약 1분 · 화면을 터치해 시작해주세요.</div>`;
}
...
function result(s, demo) {
  return `${step(3, "나의 AI 프로필", false)}${heading("YOUR NEW IDENTITY", s.aiMode === "A" ? "반가워요, 또 다른 나." : "나만의 프로필이 완성됐어요.", s.aiMode === "A" ? "8명의 캐릭터 중 가장 가까운 캐릭터예요." : "사원증에 들어갈 프로필을 확인해주세요.")}<div class="result-pass"><div class="pass-header">MIRRORTING WORKS ${logo()}</div><div class="pass-photo"><img src="${escape(s.result.image)}" alt="${demo ? "체험용 샘플 프로필" : "나의 AI 프로필"}"></div><div class="pass-info"><div><h2>${escape(s.name)}</h2><p>${s.team.title}</p></div><div class="barcode" aria-hidden="true"></div></div><div class="pass-number">${demo ? "SAMPLE PROFILE / NOT AN AI RESULT" : "EMPLOYEE PROFILE / MIRRORTING WORKS"}</div></div><p class="result-explanation">${demo ? "화면 확인을 위한 고정 샘플이며 실제 분석 결과가 아니에요." : s.aiMode === "A" ? "캐릭터 간 상대적인 특징을 비교한 결과예요." : "촬영한 얼굴을 바탕으로 완성한 프로필이에요."}</p>${actions(btn("nfc-open", "이 프로필로 사원증 만들기", "", "badge") + helper("다음 단계에서 사원증 카드를 등록해요.", "nfc"))}`;
}
...
function report(s, demo) {
  const r = s.report;
  return `${step(3, "퇴근 리포트 미리보기")}${heading("YOUR DAY, ON PAPER", "오늘의 나를 기록했어요.", demo ? "내용과 시간은 화면 확인을 위한 예시예요." : "오늘 발견한 나의 가능성을 한 장에 담았어요.")}<article class="receipt"><div class="receipt-brand">MIRRORTING WORKS</div><div class="receipt-label">${demo ? "SAMPLE / " : ""}DAILY EXPERIENCE REPORT</div><div class="receipt-row"><span>이름 / 팀</span><strong>${escape(s.name)} / ${escape(s.team.title)}</strong></div><div class="receipt-row"><span>사원 번호</span><strong>${escape(s.sessionId)}</strong></div>${r.checkinAt ? `<div class="receipt-row"><span>입사</span><strong>${escape(r.checkinAt)}</strong></div>` : ""}${r.checkoutAt ? `<div class="receipt-row"><span>퇴근</span><strong>${escape(r.checkoutAt)}</strong></div>` : ""}<hr class="receipt-rule">${[
    ["오늘의 시나리오", r.scenario],
    ["경험 돌아보기", r.summary],
    ["발견한 강점", r.strength],
